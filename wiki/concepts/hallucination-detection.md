...
- **Process-based (decompose-and-localize)** — treat detection as a *pipeline* and evaluate each stage. [[probe-process-benchmark-hallucination-detection-2026|PROBE]] decomposes it into **claim decomposition → evidence finding → evidence evaluation → hallucination localization**, scoring each step so a failure can be attributed to the breaking stage. This mirrors the reasoning-eval shift from outcome-level to **step-level / process supervision** (ProcessBench, PRMBench, FG-PRM).
